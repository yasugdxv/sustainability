...
4. needs_web_verification: 記事が、具体的な数値・日付・固有名詞を伴う一次事実
   （規制当局の発表、企業の公式発表、統計機関の統計、国際機関の文書等）を
   挙げており、かつその一次事実が誤りだった場合にこの記事の評価が変わる
   ほど重要な場合にのみtrueとすること。「不確実だから」「珍しいから」
   「重要そうだから」という理由だけでtrueにしてはいけない。決算発表や
   企業方針を一般論的に言い換えただけの記述（例:「引き続き資源の効率的な
   利用に取り組む」等、具体的な数値目標や新規発表を伴わない継続方針の
   表明）は検証対象にしないこと。記事自体が一次情報源そのものである場合や、
   検証可能な一次事実への言及が無い場合はfalse。
5. primary_source_claim: needs_web_verification=trueの場合のみ、検証すべき
   ...
   一般的な方針表明や紋切り型の言い回しはここに書かないこと（該当する場合は
   needs_web_verification自体をfalseにする）。false の場合はnull。
...
11. needs_review: 「人間による確認が必要な業務上の条件」に該当する場合にtrueとする（単に
    モデル自身の自信が低いというだけでは該当しない）。該当例:
    - 重要な主張を裏付けるだけの根拠が本文中に不足している、または主張と根拠の対応関係が弱い
    - 本文から評価の拠り所となる実質的な内容がほとんど・全く得られず、スコアが機械的に
      最低評価にならざるを得ない（例: ペイウォール定型文・エラーページ等で本文が実質的に
      空に近い）。この場合、「重要度がゼロだと確定した」と断定せず、記事取得自体が
      失敗している可能性を含め人間の確認が必要な状態として扱うこと
    - 記述内容に矛盾がある
    - 内容に曖昧さがあり複数の解釈が成立し得る
    - 後続の判断（掲載可否等）に影響し得る不確実性がある
    一方、記事が短い・情報量が少ない・話題が珍しい、というだけ（上記のいずれにも該当しない
    場合）ではtrueにしないこと。needs_web_verification（外部照合の要否）とは別軸であり、
    外部照合が不要な場合でも根拠不足・実質的内容の欠如であればneeds_review=trueになり得る。
...
- 「検索しても一致する情報が見つからなかった（未確認）」ことと、「検索で
  主張と矛盾する情報が見つかった（反証された）」ことは全く別の結論である。
  どちらに基づく判断かを必ず区別し、見つからなかっただけの場合を「誤りだと
  確認された」かのように書かないこと。

出力は次の2部構成にすること:
1. まず1〜3文の日本語のプレーンな説明文で、検証対象の主張が何であったかに
   触れた上で、検索で確認できた内容と参照した情報源、および結論が「一致する
   情報が見つかったこと」「矛盾する情報が見つかったこと」「どちらも見つから
   なかったこと」のどれに基づくかを、通常の文章として自然に言及すること
